        ...
        <div className="max-w-7xl mx-auto mb-[200px]">
            <div className="flex items-center justify-between mb-6 px-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-[#2F80ED] text-white rounded-lg hover:bg-[#2563EB] transition-colors font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isDownloadingPDF ? (
                            <>
            ...
            <div className="bg-white rounded-lg border border-[#E4E7EC] overflow-hidden mx-6" ref={invoiceRef}>
                <div className="relative z-0 p-4 sm:p-6 md:p-8 bg-white">
                            ...
                            <tbody>
                                {invoice.items?.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="py-3 px-4 text-sm text-[#101828] text-right border border-[#D0D5DD]">{formatCurrency(item.rate, invoice.currency)}</td>
                                        <td className="py-3 px-4 text-sm font-medium text-[#101828] text-right border border-[#D0D5DD]">{formatCurrency(item.amount, invoice.currency)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <div className="w-full md:w-80">
                            ...
                            <div className="mt-3 px-4 py-3 rounded" style={{ backgroundColor: invoice.invoiceColor ? `${invoice.invoiceColor}20` : '#EBF5FF' }}>
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-base font-bold text-[#101828]">{formatCurrency(invoice.totalDue, invoice.currency)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {(invoice.note || invoice.termsAndConditions || invoice.accountNumber || invoice.accountName || invoice.bank) && (
...
export default ReceivedInvoiceViewPage;
