...
const ClientModal = ({ isOpen, formData, onClose, onSave, onChange, isLoading = false, isEdit = false, error = null }: ClientModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/50 bg-opacity-30 flex items-center justify-center z-[100] p-2 md:p-3">
            <div className="bg-white rounded-lg w-full max-w-2xl flex flex-col relative max-h-[90vh]">
                {/* Header */}
                <div className="p-4 md:p-6 border-b border-[#E4E7EC] flex-shrink-0">
                    <button 
                        onClick={onClose}
                        className="absolute top-4 md:top-6 right-4 md:right-6 text-[#667085] hover:text-[#101828]"
                        disabled={isLoading}
                    >
                        <X size={20} />
                    </button>
                    ...
                    <p className="text-sm text-[#667085]">Save your client's business details to send invoices and track payments easily</p>
                </div>
