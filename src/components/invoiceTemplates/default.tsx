...
export default function DefaultInvoiceTemplate({ data = defaultInvoiceData }: { data?: InvoiceData }) {
  const { t } = useTranslation();
  
  return (
    <div className="min-h-screen bg-gray-50 py-4 px-4 sm:py-6 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto mb-4 sm:mb-6 flex justify-end gap-2 sm:gap-3">
        <button className="p-2 sm:px-4 sm:py-2 border border-blue-300 rounded-lg hover:bg-blue-50 transition-colors flex items-center gap-2 text-blue-600">
          <Edit className="w-4 h-4 sm:w-5 sm:h-5" />
          <span className="hidden sm:inline text-sm">{t('edit')}</span>
        </button>
        <button className="p-2 sm:px-4 sm:py-2 border border-blue-300 rounded-lg hover:bg-blue-50 transition-colors flex items-center gap-2 text-blue-600">
          <Download className="w-4 h-4 sm:w-5 sm:h-5" />
          <span className="hidden sm:inline text-sm">{t('download_pdf')}</span>
        </button>
        <button className="px-3 py-2 sm:px-6 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 text-sm sm:text-base">
          <Mail className="w-4 h-4 sm:w-5 sm:h-5" />
          <span>{t('email')} {t('invoice')}</span>
        </button>
      </div>

      {/* Invoice Container */}
      <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-lg p-6 sm:p-8 lg:p-12">
        {/* Header */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
          {/* Bill From */}
          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('bill_from')}</h3>
            <p className="text-base font-medium text-gray-900">{data.billFrom.name}</p>
          ...
          <div className="space-y-6">
            ...
            <div className="lg:text-right">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('bill_to')}</h3>
              <p className="text-base font-medium text-gray-900">{data.billTo.name}</p>
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left text-nowrap py-3 px-2 text-xs sm:text-sm font-semibold text-gray-600 w-12">#</th>
                <th className="text-left text-nowrap py-3 px-2 text-xs sm:text-sm font-semibold text-gray-600">{t('item_detail')}</th>
                <th className="text-right text-nowrap py-3 px-2 text-xs sm:text-sm font-semibold text-gray-600 w-16 sm:w-20">{t('qty')}</th>
                <th className="text-right text-nowrap py-3 px-2 text-xs sm:text-sm font-semibold text-gray-600 w-20 sm:w-24">{t('rate')}</th>
                <th className="text-right text-nowrap py-3 px-2 text-xs sm:text-sm font-semibold text-gray-600 w-24 sm:w-32">{t('amount')}</th>
              </tr>
            </thead>
            <tbody>
              {data.items.map((item) => (
                <tr key={item.id} className="border-b border-gray-100">
                  <td className="py-4 px-2 text-xs sm:text-sm text-gray-700">{item.id}</td>
                  <td className="py-4 px-2 text-xs sm:text-sm text-gray-900">{item.detail}</td>
                  <td className="py-4 px-2 text-xs sm:text-sm text-gray-700 text-right">{item.qty.toLocaleString()}</td>
                  <td className="py-4 px-2 text-xs sm:text-sm text-gray-700 text-right">{item.rate.toLocaleString()}</td>
                  <td className="py-4 px-2 text-xs sm:text-sm text-gray-900 text-right font-medium">
        ...
        <div className="flex justify-end mb-8 sm:mb-12">
          <div className="w-full sm:w-80 space-y-2">
            ...
            <div className="flex justify-between text-sm py-2">
              <span className="text-gray-600">{t('vat_label')} (7.5%)</span>
              <span className="text-gray-900">{formatCurrency(data.vat)}</span>
            </div>
            <div className="flex justify-between text-sm py-2">
              <span className="text-gray-600">{t('wht_label')} (5%)</span>
              <span className="text-gray-900">{formatCurrency(data.wht)}</span>
            </div>
            <div className="flex justify-between text-sm py-2 border-t border-gray-200 pt-2">
              ...
              <span className="text-gray-900 font-medium">{formatCurrency(data.total)}</span>
            </div>
            <div className="flex justify-between text-base py-3 bg-blue-50 px-4 rounded-lg">
              <span className="text-gray-800 font-semibold">{t('balance_due')}</span>
              <span className="text-gray-900 font-bold">{formatCurrency(data.balanceDue)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end mt-8 sm:mt-12">
          <button className="w-full sm:w-auto px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 text-sm sm:text-base font-medium">
