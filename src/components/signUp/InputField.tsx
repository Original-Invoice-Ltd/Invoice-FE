...
export default function InputField({
  ...
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-medium text-[#000000] mb-2 tracking-[-1px]">
        {label}
      </label>
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 pr-12 border border-[#E5E5E5] 
          rounded-lg focus:outline-none focus:ring-1
          focus:border-transparent text-sm sm:text-base text-[#B0B3B5]"
          required={required}
        />
        {type === 'password' && onTogglePassword ? (
          <button
            ...
            className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#333436] 
           transition-colors"
          ...
          icon && (
            <div className="absolute right-4 top-1/2 transform -translate-y-1/2 text-[#333436] pointer-events-none">
              {icon}
            </div>
          )
        )}
